    ...
    GLfloat slewLoc, GLfloat slewOri )
{
    ...
    for( int i = 0; i < m_particles.size(); i++ )
    {
        ...
        e = m_particles[i];
        // set goal
        e->X.update( MoFun::rand2f(-yzBound, yzBound), slewLoc );
        e->Y.update( MoFun::rand2f(-yzBound, yzBound), slewLoc );
        e->Z.update( MoFun::rand2f(0, xUpBound), slewLoc );
        e->oriX.update( MoFun::rand2f(-oBound, oBound), slewOri );
        e->oriY.update( MoFun::rand2f(-oBound, oBound), slewOri );
        e->oriZ.update( MoFun::rand2f(-oBound, oBound), slewOri );
    }
}
